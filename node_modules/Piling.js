...
var recursion = function(){
         ...
         * 寻找fn过程中的return
         */
        function findReturn(stmt, opts){
            var isPlain = utility.isPlainObject(stmt),
                isArray = utility.isArray(stmt);
            if((!isPlain || !stmt.type) && !isArray ){return;}
            (isPlain ? Object.keys(stmt) : stmt).forEach(function(item, index){
                isPlain && (item = stmt[item]);
                findReturn(item, opts);
            });
            if(stmt.type === Syntax.ReturnStatement){
                var args = stmt.argument,
                    expr = getFn('_fnEnd_', opts.codeId, opts.fnIdent).expression;;
                if(args){
                    if(args.type === Syntax.SequenceExpression){
                        args.expressions.unshift(expr);
                    }else{
                        stmt.argument = {"type": "SequenceExpression", "expressions": [expr, args]};
                    }
                }else{
                    stmt.argument = expr;
                }
        ...
        /**
         * mapping: mapping = {1: {1: {start: 10}, 2: {end: 10}, 3: {start: 10, end: 5}}}
         */
        function rec(stmt, codeId, mapping, opts){
            var isPlain = utility.isPlainObject(stmt);
            if(!utility.isArray(stmt) && !isPlain){return;}
            var type = stmt.type,
                ...
                item;
            mapping = mapping || {};
            opts = opts || {fnStmt: []};//收集fn信息
            (isPlain ? Object.keys(stmt) : stmt).forEach(function(item, index){
                isPlain && (item = stmt[item]);
                rec(item, codeId, mapping, opts);
            });
...
//                    setMapping(mapping, stmt.loc, {key: stmt.loc.end.line, merge: merge});
                ...
                case Syntax.FunctionExpression:
                    var block = stmt.body.body,
                        fnIdent = utility.getIdent();
                    block.unshift(getFn('_fnStart_', codeId, fnIdent));
                    findReturn(stmt, {codeId: codeId, fnIdent: fnIdent});
                    block.push(getFn('_fnEnd_', codeId, fnIdent));
                    opts.fnStmt.push({//收集fn信息
                        fnId: fnIdent,
                        codeId: codeId,
                        name: '-',
                        loc: {start: stmt.loc.start.line, end: stmt.loc.end.line}
                    });
                    break;
                default: break;
            }
            ...
            body.forEach(function(val, index){
                ...
                switch(val.type){
                    ...
                    case Syntax.ReturnStatement:
                        ...
                        if(args){
                            if(args.type === Syntax.SequenceExpression){
                                ...
                                val.argument = {"type": "SequenceExpression", "expressions": [expr, args]};
                            }
                        }else{
                            val.argument = expr;
                        }
                    ...
                    case Syntax.FunctionDeclaration:
                        var block = val.body.body,
                            fnIdent = utility.getIdent();
                        block.unshift(getFn('_fnStart_', codeId, fnIdent));
                        findReturn(val, {codeId: codeId, fnIdent: fnIdent});
                        block.push(getFn('_fnEnd_', codeId, fnIdent));
                        opts.fnStmt.push({//收集fn信息
                            fnId: fnIdent,
                            codeId: codeId,
                            name: val.id ? val.id.name : '-',
                            loc: {start: val.loc.start.line, end: val.loc.end.line}
                        });
                        break;
                    default: break;
                }
            ...
            stmt[prop] = clone;
            return {ast: stmt, mapping: mapping, opts: opts};
        }
        return rec;
    }();
